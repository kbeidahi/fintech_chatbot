"""Wallet models for balances, transactions, and payments."""
import uuid
...
DEFAULT_WALLET_BALANCE = Decimal("1000.00")
...
def transfer_funds(sender_user, receiver_user, amount, description=""):
    """Atomically transfer money between two user wallets."""
        ...
        if not sender_wallet.can_debit(amount):
            return (
                ...
                (
                    ...
                    f"{sender_wallet.balance} {sender_wallet.currency}."
                ),
                None,
            )
...
def process_payment(user, amount, payment_type, description="", reference=""):
    """Process bill payments, phone top-ups, purchases, and withdrawals."""
    if amount <= 0:
        return False, "Amount must be greater than zero.", None
    ...
    with transaction.atomic():
        ...
        if not wallet.can_debit(amount):
            return (
                False,
                f"Insufficient balance. Your balance is {wallet.balance} {wallet.currency}.",
                None,
            )
